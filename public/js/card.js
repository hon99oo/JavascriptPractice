...
function Card(list, title) {
	...
	this.titleNode = this.node.getElementsByClassName('card_title')[0]

	this.node.classList.add('card')
	this.node.setAttribute('card_id', this.id)
	...
	this.node.ondrop = (function (board) {
		return function (evt) {
			...
			var targetId = this.getAttribute('card_id')
			var source = board.cards[id]
			var target = board.cards[targetId]
